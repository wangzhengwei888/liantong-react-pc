...
class PeoductSearchTwo extends Component {
 constructor(props) {
  super(props)
  this.state = {
 ...
 addCarshop = (goodsId, channelPrice, count) => {
  this.props.dispatch({type: 'goodsListTwo/addCartEFF', goodsId: goodsId, goodsPrice: channelPrice, count: count});
 }


//加入收藏
 addGoodsFavorites = (goodsId) => {
  this.props.dispatch({type: 'goodsListTwo/addGoodsFavoritesEFF', goodsId: {goodsId: goodsId}})
 }
 ...
 goInquiry = (goodsId, count) => {
  ...
  this.props.dispatch({type: 'goodsListTwo/addInquiryEFF', val: {goodsId: goodsId, num: count}});
 }
 ...
 onChange = (pageNumber) => {
  this.setState({
   ...
   type: 'goodsListTwo/goodsListEFF',
 ...
 goPage = () => {
  let maxPage = Math.ceil(this.props.goodsListTwo.data[0].totalRows / 10)
 ...
 goodsSort = (sortField) => {
  if (sortField) {
   ...
   this.setState({
    sortField: sortField,
    update: true
   }, () => this.props.dispatch({type: 'goodsListTwo/goodsListEFF', val: {sortField: sortField, sortOrder: '',pageNo:1}}))
 ...
 deleteGoodsFavorites = (goodsId) => {
  Modal.confirm({
   ...
   content: '',
   onOk: () => {
    this.props.dispatch({type: 'goodsListTwo/deleteGoodsFavoritesEFF', goodsId: {goodsId: goodsId}});
   },
   onCancel() {
 ...
 onFilter1 = (specs) => {
  ...
  if (this.props.goodsListTwo.filter.brandName == specs) {
   specs = ''
  }

  this.props.dispatch({type: 'goodsListTwo/goodsListEFF', val: {brandName: specs, pageNo: 1}})
  this.props.dispatch({type: 'goodsListTwo/goodsfilterEFF', val: {brandName: specs}})
 }

 onFilter2 = (specs) => {
  if (this.props.goodsListTwo.filter.secondGcId == specs) {
   specs = ''
  }
  this.props.dispatch({type: 'goodsListTwo/goodsListEFF', val: {gcSecondId: specs,pageNo: 1}})
  this.props.dispatch({type: 'goodsListTwo/goodsfilterEFF', val: {secondGcId: specs}})
 }

 onFilter3 = (specs) => {
  if (this.props.goodsListTwo.filter.thirdGcId == specs) {
   specs = ''
  }
  this.props.dispatch({
   type: 'goodsListTwo/goodsListEFF',
  ...
  this.props.dispatch({
   type: 'goodsListTwo/goodsfilterEFF',
   val: {thirdGcId: specs}
  })
 }
 ...
 onFilter4 = (specs) => {
  if (this.props.goodsListTwo.filter.goodsSpec == specs) {
   specs = ''
  }
  this.props.dispatch({
   type: 'goodsListTwo/goodsListEFF',
 ...
 onFilter5 = (specs) => {
  if (this.props.goodsListTwo.filter.arrivalCycle == specs) {
   specs = ''
  }
  this.props.dispatch({
   type: 'goodsListTwo/goodsListEFF',
 ...
 render() {
  ...
  const goodsList = this.props.goodsListTwo;
  let {shopListObj} = goodsList;
  // console.log(this.props.goodsListTwo)
  return (
      ...
      <div>
       ...
       {/*分页*/}
       {
        goodsList.data.length > 0 && goodsList.data[0].totalRows ?
         <div className="cantent_paging" ref="page" key={goodsList.pageNo} style={{width: '96%'}}>
          <Pagination showQuickJumper
                      defaultCurrent={1}
                      defaultPageSize={10}
                      current={goodsList.pageNo}
                      total={goodsList.data.length > 0 && goodsList.data[0].totalRows ? goodsList.data[0].totalRows : 0}
...
export default connect(({goodsListTwo, app}) => ({goodsListTwo, app}), (dispatch, own) => {
 return {dispatch, own}
})(PeoductSearchTwo);
