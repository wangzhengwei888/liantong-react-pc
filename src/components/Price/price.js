    ...
    return(
      <div className={price} >
        <div className={this.state.useClass+'-'+this.state.isLogin.toString()} onClick={this.clickHandle}>
         {Number(this.props.scj) ? (Number(this.props.pj)> 0 ? (Number(this.props.pj).toFixed(2) + "  CNY") : (this.props.bz ? (Number(this.props.bj) + this.props.bz) : <span>请询价</span> )) : (Number(this.props.pj) > 0 ? (this.state.isLogin ? (Number(this.props.pj).toFixed(2) + "  CNY") : <span onClick={this.goLogin}>登录可见</span>) : <span>请询价</span>)}
        </div>
        {/*<Modal*/}
          {/*title="没有采购权限"*/}
          {/*visible={visible}*/}
